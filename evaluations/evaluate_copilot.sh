...
python evaluations/generate_eval_input_copilot.py
...
if [ "$SKIP_EVAL" = false ]; then
  ...
  if [ -n "${COMMIT_ID:-}" ]; then
    ...
    COMMIT_ID="$SHORT_COMMIT_ID" python evaluations/evaluate_copilot.py
  else
    python evaluations/evaluate_copilot.py
  fi
else
